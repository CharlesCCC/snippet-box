...
export const protect = asyncWrapper(
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    if (!token) {
      ...
      const snippetIdMatch = req.path.match(
    ...
    try {
      ...
      if (req.originalUrl.startsWith("/api/")) {
        return next(
          new ErrorResponse(401, "Invalid or expired authentication token")
        );
      }
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    if (token) {
      try {
        ...
        console.log(
          "Optional Auth middleware - Invalid token, continuing as unauthenticated"
        );
